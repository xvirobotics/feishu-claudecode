...
- **Manager admin panel** (opt-in): separate PM2 app `metabot-manager` listening on `MANAGER_PORT` (default `11000`). Bootstraps `~/.metabot/manager.env` on first run with a random admin password (printed once to PM2 stderr log) + JWT secret. Web UI at `/web/manager/` lets the admin start/stop/restart any bot on this host, edit `workingDirectory`/env/session bindings in `bots.json`, and tail PM2 logs over WebSocket. Auth = `mb_mgr_session` HttpOnly cookie. Run with `pm2 startOrReload ecosystem.config.cjs --only metabot-manager`; expose publicly via a Cloudflared anonymous tunnel pointing at `127.0.0.1:11000`. Two opt-in tweaks in `~/.metabot/manager.env`: `MANAGER_DISABLE_AUTH=true` skips login entirely (intended for `127.0.0.1`-only deployments); `MANAGER_BIND_HOST=0.0.0.0` exposes on the LAN so phones on the same Wi-Fi can hit it (UI is mobile-responsive — Dashboard collapses to a card list, BotDetail tabs scroll horizontally; combine with disable-auth only on a trusted LAN). BotDetail's `Sessions` tab lists chat sessions ChatGPT-style; click a row to open `/web/transcript/:chatId?turn=all` in a new tab. Per-bot `hubVisible: true` in `bots.json` (toggle on BotDetail Overview) opts the bot into the **Hub UI** (next bullet).
- **Hub UI** (opt-in): central-server view of every `hubVisible: true` bot on this host, served by `metabot-manager` at `/web/hub/`. Endpoints: `GET /api/hub/hosts`, `GET /api/hub/hosts/:hostId`, `GET /api/hub/bots/:name/sessions`. Auth = `mb_session` (Feishu OAuth, the same cookie the Transcript page uses); whitelist is the union of every hubVisible bot's `accessAllowOpenIds` (owner-identity gate). Bots without `hubVisible` are hidden but counted in `hiddenBotCount`; sensitive fields (`feishuAppSecret`, `env`) are never serialized. Single-machine in Phase 1 — `hostId = os.hostname().toLowerCase()`. Click a bot → session list → opens the bot's `publicBaseUrl/web/transcript/...` in a new tab. The middleware `requireFeishuAuth` in `src/api/middleware/feishu-auth.ts` is shared with the Transcript route; `accessAllowOpenIds` is the new per-bot owner whitelist (transcript routes still also honor the legacy `transcriptAllowOpenIds` + `METABOT_TRANSCRIPT_ALLOW_OPEN_IDS` env fallback).
