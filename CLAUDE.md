...
- **Manager admin panel** (opt-in): separate PM2 app `metabot-manager` listening on `MANAGER_PORT` (default `11000`). Bootstraps `~/.metabot/manager.env` on first run with a random admin password (printed once to PM2 stderr log) + JWT secret. Web UI at `/manager/` lets the admin start/stop/restart any bot on this host, edit `workingDirectory`/env/session bindings in `bots.json`, and tail PM2 logs over WebSocket. Auth = `mb_mgr_session` HttpOnly cookie. Run with `pm2 startOrReload ecosystem.config.cjs --only metabot-manager`; expose publicly via a Cloudflared anonymous tunnel pointing at `127.0.0.1:11000`.
