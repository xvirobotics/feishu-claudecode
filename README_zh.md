...
| **IM Bridge** | 飞书、Telegram 或微信（含手机端）与任意 Agent 对话。带颜色状态的流式卡片/消息 + 工具调用追踪。 |
...
**微信**（通过 iLink）：
1. 通过 iLink QR 码登录获取 bot token
2. 在 `.env` 中设置 `WECHAT_BOT_TOKEN=your-token`，或添加到 `bots.json` 的 `wechatBots`
3. 仅支持私聊（不支持群聊）。消息带打字指示器流式展示。
...
刚接触 MetaBot？以下是你可以直接在飞书/Telegram/微信中发送的真实 prompt，帮你解锁高级功能。
...
| `wechatBotToken` | 微信 | — | iLink Bot Token |
...
MetaBot 内置了 Claude Code 技能，简化贡献流程。可以在终端或飞书/Telegram/微信聊天中直接使用。
...
**飞书/Telegram/微信使用：** 直接在聊天中发送相同命令（如 `/report-bug ...`）。Bot 会将命令转发给 Claude Code，以交互式卡片展示过程和结果。
